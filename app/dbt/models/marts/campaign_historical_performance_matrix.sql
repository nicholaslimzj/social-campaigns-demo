...
/*
Model: campaign_historical_performance_matrix
...
to identify optimal targeting strategies across all historical campaign data.

Key Features:
1. Creates a complete matrix of all possible goal-segment combinations
2. Calculates key performance metrics for each combination:
   - Conversion Rate: How effectively campaigns convert for each goal-segment combination
   - ROI: Return on investment for each goal-segment combination
   - Acquisition Cost: Cost efficiency for each goal-segment combination
   - CTR: Engagement levels for each goal-segment combination
3. Provides multi-dimensional performance comparisons (vs. goal average, segment average, global average)
4. Implements a weighted composite scoring system to identify top performers

Dashboard Usage:
- Campaign Performance Matrix visualization
- Goal-Segment Optimization recommendations
*/
