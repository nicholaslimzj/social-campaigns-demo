...
/*
Model: dimensions_quarter_performance_rankings
Time Context: Current Quarter
Time Range: Last 3 months of available data with quarter-over-quarter comparison

Description:
This model ranks all entities (companies, channels, segments, target audiences) within their respective dimensions
based on key performance metrics for the current quarter, with quarter-over-quarter comparison.
...
-- Current quarter audience metrics
current_audience_metrics AS (
    SELECT
        'audience' AS dimension,
        Target_Audience AS entity,
        COUNT(*) AS campaign_count,
        AVG(Conversion_Rate) AS avg_conversion_rate,
        AVG(ROI) AS avg_roi,
        AVG(Acquisition_Cost) AS avg_acquisition_cost,
...
-- Previous quarter audience metrics
previous_audience_metrics AS (
    SELECT
        'audience' AS dimension,
        Target_Audience AS entity,
        COUNT(*) AS campaign_count,
...
current_all_metrics AS (
    ...
    UNION ALL
    SELECT * FROM current_audience_metrics
),
...
previous_all_metrics AS (
    ...
    UNION ALL
    SELECT * FROM previous_audience_metrics
),
